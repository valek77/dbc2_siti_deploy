...
$pageTitle = 'Offerte Luce e Gas Green';
$pageDescription = 'Scegli la sostenibilità e la convenienza. Scopri le tariffe ecologiche di Locura per l\'energia elettrica rinnovabile e il gas naturale a impatto neutro.';
  ...
  <main class="section" style="padding: 80px 0 40px;">
    <div class="container">
      ...
      <div class="tab-bar" id="tab-bar">
        ...
        <button class="tab-btn" data-filter="luce-res">Luce</button>
        <button class="tab-btn" data-filter="gas-res">Gas</button>
      </div>

      <!-- Griglia offerte -->
      <div id="offers-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(340px, 1fr)); gap: 32px;"></div>

      <p style="font-size: 13.5px; color: var(--muted); text-align: center; max-width: 900px; margin: 60px auto 0; line-height: 1.7;">
        * Oltre ai corrispettivi sopra citati per le offerte Luce e Gas, verranno applicati i corrispettivi previsti dall'Autorità. I prezzi della componente energia e gas restano bloccati per i primi 3 anni (36 mesi). Il Bonus Bundle di 20€ viene riconosciuto in caso di attivazione congiunta Luce + Gas e ripartito in due quote. Locura opera come partner autorizzato per la transizione energetica.
      </p>
    </div>
  </main>
...
$pageScripts = <<<'HTML'
  <script>
    ...
    const offers = [
      { id: 'offer-0', category: 'luce-res', kind: 'luce', tipo: 'Luce', top: true,
        nome: 'Solo Web - Variabile PUN', sub: 'Domestico · Offerta Solo Web',
        prezzoRid: 'PUN + 0,055', unita: '€/kWh', prezzoBoll: null,
        note: 'Corrispettivo annuo: 456 €/POD/anno',
        features: ['Prezzo indicizzato al PUN Index GME con spread fisso per 12 mesi.', '1,00 € sconto bolletta via e-mail', '1,00 € sconto addebito SDD'] },
      { id: 'offer-2', category: 'luce-res', kind: 'luce', tipo: 'Luce · Logicard', top: false,
        nome: 'Solo Web - Variabile PUN Logicard Casa', sub: 'Domestico · Con carta Logicard Casa',
        prezzoRid: 'PUN + 0,05', unita: '€/kWh', prezzoBoll: null,
        note: 'Corrispettivo annuo: 456 €/POD/anno',
        features: ['Indicizzata al PUN Index GME con carta Logicard Casa (contributo 168 €+IVA/anno).', '1,00 € sconto bolletta via e-mail', '1,00 € sconto addebito SDD'] },
      { id: 'offer-3', category: 'gas-res', kind: 'gas', tipo: 'Gas', top: false,
        nome: 'Solo Web - Variabile PSV', sub: 'Domestico · Offerta Solo Web',
        prezzoRid: 'PSV + 0,45', unita: '€/Smc', prezzoBoll: null,
        note: 'Corrispettivo annuo: 648 €/PDR/anno',
        features: ['Prezzo all\'ingrosso indicizzato all\'indice PSV Day Ahead, aggiornato mensilmente.', '1,00 € sconto bolletta via e-mail', '1,00 € sconto addebito SDD'] },
      { id: 'offer-4', category: 'gas-res', kind: 'gas', tipo: 'Gas · Logicard', top: false,
        nome: 'Solo Web - Variabile PSV Logicard Casa', sub: 'Domestico · Con carta Logicard Casa',
        prezzoRid: 'PSV + 0,42', unita: '€/Smc', prezzoBoll: null,
        note: 'Corrispettivo annuo: 648 €/PDR/anno',
        features: ['Indicizzata al PSV con carta Logicard Casa (contributo 168 €+IVA/anno).', '1,00 € sconto bolletta via e-mail', '1,00 € sconto addebito SDD'] }
    ];

    function renderCard(o) {
      const isGas = o.kind === 'gas';
      const styleVars = isGas
        ? '--ribbon-color:#d97706; --ribbon-bg:#fef3c7; --ribbon-text:#b45309; --ribbon-border:#fde68a;'
        : '';
      return `
      <article class="offer-card ${o.top ? 'featured' : ''}" data-category="${o.category}" style="${styleVars}">
        ...
        <div class="offer-card-body">
          ...
          <div class="price-block">
            <div class="price-label">Prezzo componente · con RID</div>
            ...
            ${o.prezzoBoll
              ? `<div class="price-alt">Bollettino postale: <b>${o.prezzoBoll}</b></div>`
              : `<div class="price-locked">${ICON_CHECK} Tariffa PLACET, spread garantito 12 mesi</div>`}
          </div>
          ...
          <button class="btn-primary" data-offer="${o.id}" data-name="${o.nome}">Richiedi attivazione
