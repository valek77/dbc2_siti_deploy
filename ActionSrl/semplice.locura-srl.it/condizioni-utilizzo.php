...
// Titolarità/proprietà del Sito: appartiene all'OPERATORE energetico, non all'azienda.
$proprietarioSito = $OPERATORE['nome_legale'] !== '' ? $OPERATORE['nome_legale']
    : ($OPERATORE['nome_marketing'] !== '' ? $OPERATORE['nome_marketing'] : 'Illumia');
$proprietarioSede = $OPERATORE['indirizzo'];
$proprietarioPiva = $OPERATORE['partita_iva'];
...
?>
  ...
  <main class="privacy-box">
    ...
    <h2>Premessa</h2>
    <p>L’utilizzo del presente sito web<?php if ($LANDING_PAGE['url'] !== '') { ?> <?= $LANDING_PAGE['url'] ?><?php } ?> (di seguito, il “Sito”) comporta l’accettazione integrale delle presenti condizioni generali di utilizzo. Il Sito è di titolarità e proprietà di <strong><?= $proprietarioSito ?></strong><?php if ($proprietarioSede !== '') { ?>, con sede in <?= $proprietarioSede ?><?php } ?><?php if ($proprietarioPiva !== '') { ?>, Partita IVA <?= $proprietarioPiva ?><?php } ?>.</p>
    ...
    <p>Tutti i contenuti presenti sul Sito sono di proprietà di <strong><?= $proprietarioSito ?></strong> o dei rispettivi titolari dei diritti e sono protetti dalla normativa sulla proprietà intellettuale.</p>
