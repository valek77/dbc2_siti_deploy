...
<?php } ?>
      <p style="margin-top: 8px; font-size: 13px; opacity: 0.8;"><?php if ($COMPANY['logo2_url'] !== '') { ?><img src="<?= $COMPANY['logo2_url'] ?>" alt="<?= $COMPANY['company_name'] !== '' ? $COMPANY['company_name'] : $brandName ?>" style="height: 16px; width: auto; vertical-align: middle; margin-right: 8px;"><?php } ?>&copy; <?= date('Y') ?> <?= $legalLine !== '' ? $legalLine . '. ' : '' ?>Tutti i diritti riservati.</p>
    </div>
  </footer>
