...
Nella cartella del sito copia il file **`.env.example`** in **`.env`** e compila i valori
(si fa una sola volta; il file `.env` **non si carica su git**):

```
DBC2_API_BASE=https://dbc2.datalia.it/api
DBC2_TOKEN=<token segreto fornito da Datalia>
COMPANY_ID=<numero dell'azienda>
SITO_WEB=domestika.energyfast-srl.it
OPERATORE_ENERGETICO=Domestika Energia
```

- `COMPANY_ID` = numero dell'azienda di questo sito (es. LCtarde Call srl).
- `SITO_WEB` = indirizzo mostrato nei testi legali.
- `OPERATORE_ENERGETICO` = operatore di cui il sito è rivenditore (usato nella
  home, nella pagina Offerte, nei consensi del form e nel footer).
...
> Tutti i campi della risposta API sono disponibili allo stesso modo (anche quelli
> non in tabella). Se un campo non esiste, la variabile è **vuota** (`""`), quindi
> non dà errori.

### Mostrare qualcosa solo se il dato c'è

```php
<?php if ($telefono) { ?>
  <p>Chiamaci: <a href="tel:<?= $telefono ?>"><?= $telefono ?></a></p>
<?php } ?>
```
...
- `$pageDescription` → meta description della pagina (SEO)
- `$pageHead`        → CSS extra dentro `<head>` (vedi `condizioni-utilizzo.php`)
- `$pageScripts`     → `<script>` specifici della pagina (vedi `contatti.php`, `index.php`)
