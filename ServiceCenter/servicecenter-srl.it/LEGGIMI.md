# Guida rapida — siti PHP con dati azienda da API
...
COMPANY_ID=<numero azienda>
SITO_WEB=www.servicecenter-srl.it
OPERATORE_ENERGETICO=NUOVA CORRENTE
```
...
- `$pageHead`    → CSS extra dentro `<head>` (vedi `privacy-policy.php`)
- `$pageScripts` → `<script>` specifici della pagina (vedi `contatti.php`)
