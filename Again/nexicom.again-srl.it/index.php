...
// In questa pagina il nome del portale mostrato nella testata è quello commerciale API.
$LANDING_PAGE['titolo'] = $brand;
$opLogo = $OPERATORE['logo_url'] !== '' ? $OPERATORE['logo_url'] : $OPERATORE['logo2_url'];

$pageDescription = $brand . ' ti accompagna nella scelta di forniture luce e gas: offerte indicizzate, spread trasparenti e consulenza per casa e impresa.';

// Anteprima delle offerte reali pubblicate nella landing page.
$homeOfferte = array_slice($OFFERTE, 0, 4);
...
<style>
  ...
  .nx-link {
    ...
    color: var(--primary-700); font-size: 15px;
    text-decoration: underline; text-decoration-color: var(--accent);
    text-decoration-thickness: 2px; text-underline-offset: 4px;
  }
  .nx-link:hover { color: var(--primary); text-decoration-color: var(--primary); }
  ...
  .nx-hero-media .tag {
    ...
    background: rgba(10,33,65,.55); color: #fff; backdrop-filter: blur(8px);
  ...
  .nx-ticket:hover { transform: translateY(-6px); box-shadow: var(--shadow-lg); border-color: rgba(255,170,0,.3); }
  ...
  .nx-ticket.luce .tipo { background: rgba(255,170,0,.12); color: var(--accent-deep); }
  .nx-ticket.gas .tipo { background: rgba(21,67,131,.1); color: var(--primary); }
  ...
  .nx-calc::before { content: ''; position: absolute; inset: 0; background: rgba(255,170,0,.18); pointer-events: none; }
  ...
  .nx-calc input { flex: 1; background: rgba(10,33,65,.5); border: 1px solid rgba(255,255,255,.18); border-radius: var(--r-sm); padding: 16px 18px; color: #fff; font-family: var(--font-body); font-size: 16px; font-weight: 600; outline: none; }
  ...
  .nx-cta-inner::before { content: ''; position: absolute; inset: 0; background: rgba(255,255,255,.12); pointer-events: none; }
...
<div class="nx-home">
    ...
    <div class="nx-wrap">
      ...
      <div class="nx-hero-grid">
        <div class="reveal">
          ...
          <h1>L'energia diventa <em>più semplice</em>.</h1>
          <p class="lede">Con <?= $brand ?> capisci davvero cosa stai pagando e trovi la soluzione luce o gas più adatta alle tue abitudini. Un confronto chiaro, senza complicazioni.</p>
  ...
  <section class="nx-manifesto">
    <div class="nx-wrap reveal">
      ...
      <p class="lead">Non ti proponiamo una tariffa qualsiasi: <em>partiamo dai tuoi consumi</em>. Leggiamo la bolletta, confrontiamo le alternative e ti aiutiamo a scegliere con maggiore consapevolezza.</p>
      <div class="cols">
        <div><h4>Chiarezza</h4><p>Ti spieghiamo le componenti dell'offerta in modo semplice, così puoi valutare ogni voce prima di decidere.</p></div>
        <div><h4>Ascolto</h4><p>Analizziamo il tuo profilo di consumo per orientarti verso una proposta coerente con le tue esigenze reali.</p></div>
        <div><h4>Affidabilità</h4><p>Ti seguiamo nel percorso, dalla prima analisi alla richiesta di attivazione, con un riferimento sempre disponibile.</p></div>
      </div>
    </div>
  </section>
  ...
  <section class="nx-offers">
    <div class="nx-wrap">
      <div class="head reveal">
        ...
        <h2 class="section-title" style="text-align:left; margin:18px 0 0;">Scegli con <em style="font-style:italic;color:var(--primary);">più consapevolezza</em>.</h2>
        </div>
        <p class="partner">Con <strong><?= $brand ?></strong> confronti offerte trasparenti e adatte ai tuoi consumi.</p>
      </div>
      <div class="nx-offer-grid">
<?php foreach ($homeOfferte as $o):
        $isLuce = strpos(strtolower($o['tipologia']), 'gas') !== 0;
        $href = 'contatti.php?offerta=' . rawurlencode((string) $o['id']) . '#contatto-form';
?>
        <a class="nx-ticket <?= $isLuce ? 'luce' : 'gas' ?> reveal" href="<?= $href ?>">
<?php if ($opLogo !== '') { ?>
          <div class="nx-ticket-brand"><img src="<?= e($opLogo) ?>" alt="<?= e($brand) ?>" loading="lazy"></div>
<?php } ?>
          <div class="row">
            <span class="tipo"><?= e(dbc2_format_tipologia($o['tipologia'])) ?></span>
            <span class="code">Offerta <?= e((string) $o['id']) ?></span>
          </div>
          <h3><?= e($o['nome']) ?></h3>
<?php foreach (array_slice($o['caratteristiche_evidenza'], 0, 1) as $evidenza) { ?>
          <div class="price"><?= $evidenza ?></div>
<?php } ?>
          <?php if ($o['sottotitolo'] !== '') { ?><div class="annuo"><?= $o['sottotitolo'] ?></div><?php } ?>
  ...
  <section>
    <div class="nx-feature-row reverse reveal">
      <div class="body">
        <span class="nx-kicker">Consulenza dedicata</span>
            <h2>La tua bolletta, <em>spiegata bene</em>.</h2>
        <p>Partiamo dai tuoi consumi e mettiamo in ordine le informazioni che contano: prezzi, condizioni e modalità di attivazione. Così puoi scegliere l'offerta luce o gas più adatta.</p>
        <ul>
          <li><span style="color:var(--primary);font-weight:800;">✓</span> Lettura della bolletta e confronto delle alternative</li>
          <li><span style="color:var(--primary);font-weight:800;">✓</span> Spiegazione semplice di prezzi e condizioni</li>
          <li><span style="color:var(--primary);font-weight:800;">✓</span> Supporto nelle pratiche di attivazione</li>
        </ul>
    ...
    <div class="nx-feature-row dark reveal">
      ...
      <div class="body">
        ...
        <h2>Un team al tuo <em>fianco</em>.</h2>
        <p>Mettiamo competenza e attenzione al servizio delle tue scelte energetiche. <?= $brand ?> rende più leggibile il mercato libero e ti accompagna senza percorsi complicati.</p>
