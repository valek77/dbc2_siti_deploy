...
// Ogni recapito viene reso come link mailto cliccabile.
$contattoTitolareParts = [];
foreach (array_filter([$emailSupporto, $COMPANY['pec']]) as $recapito) {
    $contattoTitolareParts[] = '<a href="mailto:' . $recapito . '">' . $recapito . '</a>';
}
$contattoTitolare = implode(' / ', $contattoTitolareParts);
...
<?php } ?>
<?php if ($emailDpo && $emailDpo !== $emailSupporto) { ?>      <li><a href="mailto:<?= $emailDpo ?>"><?= $emailDpo ?></a></li>
<?php } ?>
...
<?php if ($emailSupporto) { ?>    <p>Per esercitare la revoca è possibile scrivere a <a href="mailto:<?= $emailSupporto ?>"><?= $emailSupporto ?></a><?= ($emailDpo && $emailDpo !== $emailSupporto) ? ' o <a href="mailto:' . $emailDpo . '">' . $emailDpo . '</a>' : '' ?>.</p>
<?php } ?>
